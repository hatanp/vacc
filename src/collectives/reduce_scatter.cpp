...
#include <sys/types.h>
#include <sys/uio.h>
int handle_recv(float* input_buf, float* comm_buf, vacc::vacc_fi_info_t* vacc_fi_info, int c_r, int p_r, int nic, int rs_end_chunk, int ring_next_nic0, size_t chunk_count, size_t chunk_elems, size_t part_chunk_elems_pipe, size_t part_chunk_elems_nic, size_t pipe_chunk_count){
    int err = 0;
    int sum_loop_threads = 32;
        ...
        while (fails < 10000 && completions < pipe_chunk_count*nic_chunk_count){
            for(int nic = 0;nic<nic_chunk_count;nic++){
                ...
                if (ret > 0) {
                    ...
                    int c_r = rx_entry.tag & 0xFF;
                    assert(c_r==c);
                    int p_r = (rx_entry.tag & 0xFFFF0000)>>16;
                    //int n_r = (entry.tag & 0xFF0000)>>16;
                    //Handle RS recv 
                    handle_recv(input_buf, comm_buf, vacc_fi_info, c_r, p_r, nic, rs_end_chunk, ring_next_nic0, chunk_count, chunk_elems, part_chunk_elems_pipe, part_chunk_elems_nic, pipe_chunk_count);
                    
                    //std::thread recv_thread(handle_recv);
                    //recv_thread.join();
